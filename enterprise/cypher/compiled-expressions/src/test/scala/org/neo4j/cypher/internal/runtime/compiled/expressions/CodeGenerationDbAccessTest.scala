...
class CodeGenerationDbAccessTest extends CypherFunSuite with AstConstructionTestSupport {
  ...
  when(dbAccess.nodeProperty(node, "prop")).thenReturn(stringValue("hello"))
  when(dbAccess.nodeProperty(node, "notThere")).thenReturn(NO_VALUE)
  when(dbAccess.relationshipProperty(relationship, "prop")).thenReturn(stringValue("hello"))
  when(dbAccess.relationshipProperty(relationship, "notThere")).thenReturn(NO_VALUE)
  when(dbAccess.nodeHasProperty(node, property)).thenReturn(true)
  when(dbAccess.nodeHasProperty(node, "prop")).thenReturn(true)
  when(dbAccess.nodeHasProperty(node, "notThere")).thenReturn(false)
  ...
  when(dbAccess.relationshipHasProperty(relationship, "prop")).thenReturn(true)
  when(dbAccess.relationshipHasProperty(relationship, "notThere")).thenReturn(false)
