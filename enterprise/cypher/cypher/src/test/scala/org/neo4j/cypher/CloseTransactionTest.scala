...
class CloseTransactionTest extends CypherFunSuite with GraphIcing {
  ...
  for (runtime <- runtimes) {
    ...
    test(s"should not leak transaction when closing the result for a query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime return 1").close()
      // then
      txBridge(service).hasTransaction shouldBe false
    ...
    test(s"should not leak transaction when closing the result for a profile query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime profile return 1").close()
      // then
      txBridge(service).hasTransaction shouldBe false
    ...
    test(s"should not leak transaction when closing the result for an explain query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime explain return 1").close()
      // then
      txBridge(service).hasTransaction shouldBe false
    ...
    test(s"should not leak transaction when consuming the whole iterator for a regular query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime return 1").resultAsString()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime return 1").asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when consuming the whole iterator for a profile query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime profile return 1").resultAsString()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime profile return 1").asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when consuming the whole iterator for an explain query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime explain return 1").resultAsString()
      // then
      txBridge(service).hasTransaction shouldBe false

      // when
      engine.execute(s"CYPHER runtime=$runtime explain return 1").asScala.length
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when visiting the result for a regular query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime return 1").accept(consumerVisitor)
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when visiting the result for a profile query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime profile return 1").accept(consumerVisitor)
      // then
      txBridge(service).hasTransaction shouldBe false
    }
    ...
    test(s"should not leak transaction when visiting the result for an explain query - runtime=$runtime") {
      ...
      // when
      engine.execute(s"CYPHER runtime=$runtime explain return 1").accept(consumerVisitor)
      // then
      txBridge(service).hasTransaction shouldBe false
    }
  ...
  implicit class RichExecutionEngine(engine: ExecutionEngine) {
    def profile(query: String): Result =
      engine.profile(query, Values.emptyMap(), engine.queryService.transactionalContext(query = query -> Map()))

    def execute(query: String): Result =
      engine.execute(query, Values.emptyMap(), engine.queryService.transactionalContext(query = query -> Map()))
  }
